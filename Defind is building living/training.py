...
def processing_line(moscow_map_data, line, words, string): 
    line_save = line
    line = proccessing_fulladdress(line, words, string)
    if line == -1: 
        return "Incorrect line: %s" % line_save
    else:
...
with open("13.csv", "r") as f_r:
    for line in f_r:
        lines.append(line)
words = ["улица", "бульвар", "переулок", "проспект", "проезд", "шоссе", "площадь", "набережная", "аллея", "тупик", "микрорайон", "квартал"]
names = ["Щербинка", "Зеленоград", "Внуково", "ТАО", "НАО", "Троицк", "Акулово",  "Толстопальцево"]
moscow_map_data = get_addresses_from_file()
with open("error.csv", "w") as f_w:
    for line in lines:
        flag = 0
        for name in names: 
            if name in line:
                line = line.replace(name + ", ", "")
                result = processing_line(moscow_map_data, line, words, name + " ")
                flag = 1
                break
        if flag == 0: result = processing_line(moscow_map_data, line, words, "")
